...
namespace StateMachine.AsyncEx {
	...
	/// <summary>
	///     A collection of cancelable <see cref="TaskCompletionSource{T}" /> instances. Implementations must assume the caller
	///     is holding a lock.
	/// </summary>
	...
	public static class AsyncWaitQueueExtensions {
		...
		public static Task<T> Enqueue<T>(this IAsyncWaitQueue<T> @this, object mutex, CancellationToken token) {
			...
			var registration = token.Register(() => {
				lock (mutex) @this.TryCancel(ret, token);
			}, false);
	...
	/// </summary>
	/// <typeparam name="T">The type of the results.</typeparam>
	[DebuggerDisplay("Count = {Count}"), DebuggerTypeProxy(typeof(DefaultAsyncWaitQueue<>.DebugView))]
	public sealed class DefaultAsyncWaitQueue<T> : IAsyncWaitQueue<T> {
		private readonly List<TaskCompletionSource<T>> _queue = new();
		...
		void IAsyncWaitQueue<T>.DequeueAll(T result) {
			foreach (var source in _queue) source.TrySetResult(result);
			_queue.Clear();
		}
		...
		void IAsyncWaitQueue<T>.CancelAll(CancellationToken cancellationToken) {
			foreach (var source in _queue) source.TrySetCanceled(cancellationToken);
			_queue.Clear();
		}
		...
		internal sealed class DebugView {
			...
			public Task<T>[] Tasks {
				get {
					...
					foreach (var entry in _queue._queue) result.Add(entry.Task);

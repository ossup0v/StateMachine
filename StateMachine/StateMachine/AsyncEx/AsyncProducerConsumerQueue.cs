...
namespace StateMachine.AsyncEx {
	...
	[DebuggerDisplay("Count = {_queue.Count}, MaxCount = {_maxCount}"), DebuggerTypeProxy(typeof(AsyncProducerConsumerQueue<>.DebugView))]
	public sealed class AsyncProducerConsumerQueue<T> {
		...
		/// <summary>
		///     Creates a new async-compatible producer/consumer queue with the specified initial elements and a maximum element
		///     count.
		/// </summary>
		/// <param name="collection">
		///     The initial elements to place in the queue. This may be <c>null</c> to start with an empty
		///     collection.
		/// </param>
		/// <param name="maxCount">
		///     The maximum element count. This must be greater than zero, and greater than or equal to the
		///     number of elements in <paramref name="collection" />.
		/// </param>
		public AsyncProducerConsumerQueue(IEnumerable<T> collection, int maxCount) {
			...
			if (maxCount < _queue.Count) throw new ArgumentException("The maximum count cannot be less than the number of elements in the collection.", nameof(maxCount));
			_maxCount = maxCount;
		...
		/// <summary>
		///     Enqueues an item to the producer/consumer queue. Throws <see cref="InvalidOperationException" /> if the
		///     producer/consumer queue has completed adding.
		/// </summary>
		...
		private async Task DoEnqueueAsync(T item, CancellationToken cancellationToken, bool sync) {
			using (sync ? _mutex.Lock() : await _mutex.LockAsync().ConfigureAwait(false)) {
				...
				if (_completed) throw new InvalidOperationException("Enqueue failed; the producer/consumer queue has completed adding.");
		...
		public Task EnqueueAsync(T item, CancellationToken cancellationToken) => DoEnqueueAsync(item, cancellationToken, false);
		...
		/// <summary>
		///     Enqueues an item to the producer/consumer queue. This method may block the calling thread. Throws
		///     <see cref="InvalidOperationException" /> if the producer/consumer queue has completed adding.
		/// </summary>
		/// <param name="item">The item to enqueue.</param>
		/// <param name="cancellationToken">A cancellation token that can be used to abort the enqueue operation.</param>
		/// <exception cref="InvalidOperationException">The producer/consumer queue has been marked complete for adding.</exception>
		public void Enqueue(T item, CancellationToken cancellationToken) {
			DoEnqueueAsync(item, cancellationToken, true).WaitAndUnwrapException();
		}
		...
		/// <summary>
		///     Waits until an item is available to dequeue. Returns <c>false</c> if the producer/consumer queue has completed
		///     adding and there are no more items.
		/// </summary>
		...
		/// <summary>
		///     Synchronously waits until an item is available to dequeue. Returns <c>false</c> if the producer/consumer queue has
		///     completed adding and there are no more items.
		/// </summary>
		/// <param name="cancellationToken">A cancellation token that can be used to abort the asynchronous wait.</param>
		public bool OutputAvailable(CancellationToken cancellationToken) => DoOutputAvailableAsync(cancellationToken, true).WaitAndUnwrapException();
		...
		///     producer/consumer queue has completed adding and is empty.
		/// </summary>
		/// <param name="cancellationToken">A cancellation token that can be used to abort the dequeue operation.</param>
		/// <param name="sync">Whether to run this method synchronously.</param>
		/// <exception cref="InvalidOperationException">
		///     The producer/consumer queue has been marked complete for adding and is
		///     empty.
		/// </exception>
		private async Task<T> DoDequeueAsync(CancellationToken cancellationToken, bool sync) {
			using (sync ? _mutex.Lock() : await _mutex.LockAsync().ConfigureAwait(false)) {
				...
				if (_completed && Empty) throw new InvalidOperationException("Dequeue failed; the producer/consumer queue has completed adding and is empty.");
		...
		///     empty.
		/// </exception>
		public Task<T> DequeueAsync(CancellationToken cancellationToken) => DoDequeueAsync(cancellationToken, false);

		/// <summary>
		///     Dequeues an item from the producer/consumer queue. Returns the dequeued item. Throws
		///     <see cref="InvalidOperationException" /> if the producer/consumer queue has completed adding and is empty.
		/// </summary>
		/// <returns>The dequeued item.</returns>
		/// <exception cref="InvalidOperationException">
		///     The producer/consumer queue has been marked complete for adding and is
		///     empty.
		/// </exception>
		public Task<T> DequeueAsync() => DequeueAsync(CancellationToken.None);

		/// <summary>
		///     Dequeues an item from the producer/consumer queue. Returns the dequeued item. This method may block the calling
		///     thread. Throws <see cref="InvalidOperationException" /> if the producer/consumer queue has completed adding and is
		///     empty.
		/// </summary>
		/// <param name="cancellationToken">A cancellation token that can be used to abort the dequeue operation.</param>
		/// <returns>The dequeued item.</returns>
		...
		///     empty.
		/// </exception>
		public T Dequeue(CancellationToken cancellationToken) => DoDequeueAsync(cancellationToken, true).WaitAndUnwrapException();
		...
		///     empty.
		/// </exception>
		public T Dequeue() => Dequeue(CancellationToken.None);

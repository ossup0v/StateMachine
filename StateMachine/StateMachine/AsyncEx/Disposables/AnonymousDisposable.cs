namespace StateMachine.AsyncEx.Disposables {
	...
	public sealed class AnonymousDisposable : SingleDisposable<Action> {
		...
		/// </summary>
		/// <param name="dispose">
		///     The delegate to execute when disposed. If this is <c>null</c>, then this instance does nothing
		///     when it is disposed.
		/// </param>
		...
		/// <summary>
		///     Adds a delegate to be executed when this instance is disposed. If this instance is already disposed, then
		///     <paramref name="dispose" /> is executed immediately.
		/// </summary>
		...
		public void Add(Action dispose) {
			...
			if (!TryUpdateContext(x => x + dispose)) dispose();
		}

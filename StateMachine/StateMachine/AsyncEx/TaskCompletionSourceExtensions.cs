namespace StateMachine.AsyncEx {
	...
	public static class TaskCompletionSourceExtensions {
		/// <summary>
		///     Attempts to complete a <see cref="TaskCompletionSource{TResult}" />, propagating the completion of
		///     <paramref name="task" />.
		/// </summary>
		...
		/// <summary>
		///     Attempts to complete a <see cref="TaskCompletionSource{TResult}" />, propagating the completion of
		///     <paramref name="task" /> but using the result value from <paramref name="resultFunc" /> if the task completed
		///     successfully.
		/// </summary>
		/// <typeparam name="TResult">The type of the result of the target asynchronous operation.</typeparam>
		/// <param name="this">The task completion source. May not be <c>null</c>.</param>
		...
		///     A delegate that returns the result with which to complete the task completion source, if the
		///     task completed successfully. May not be <c>null</c>.
		/// </param>
		/// <returns><c>true</c> if this method completed the task completion source; <c>false</c> if it was already completed.</returns>
		...
		public static TaskCompletionSource<TResult> CreateAsyncTaskSource<TResult>() => new(TaskCreationOptions.RunContinuationsAsynchronously);
	}
